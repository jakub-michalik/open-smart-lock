...
| Address range | Partition | Size | Notes |
| ---: | --- | ---: | --- |
| `0x00000` – `0xF8000` | application image | 992 KiB | debug ≈ 605 KiB · release ≈ 461 KiB |
| `0xF8000` – `0x100000` | `settings_storage` | 32 KiB | Matter fabric data, ACLs, persistent attributes |
...
| Address range | Partition | Size | Notes |
| ---: | --- | ---: | --- |
| `0x00000` – `0x07000` | `mcuboot` bootloader | 28 KiB | second-stage loader, swap-move |
| `0x07000` – `0x07200` | `mcuboot_pad` | 512 B | MCUboot image header |
| `0x07200` – `0x80000` | application (mcuboot_primary span) | 483 KiB | release+OTA build |
| `0x80000` – `0xF9000` | `mcuboot_secondary` | 484 KiB | OTA staging area |
| `0xF9000` – `0xFF000` | `settings_storage` | 24 KiB | Matter fabric data, ACLs |
| `0xFF000` – `0x100000` | `factory_data` | 4 KiB | DAC, PAI, discriminator, passcode hash, vendor/product IDs |
...
block-beta
    ...
    block:primary
        A["mcuboot (28 KiB)"]
        B["app — primary slot (484 KiB)"]
    end
    C["mcuboot_secondary — OTA staging (484 KiB)"]
    D["settings_storage (24 KiB)"]
    E["factory_data (4 KiB)"]
    style A fill:#fde4cf
    style B fill:#a8d5ba
    style C fill:#dde7f5
    style D fill:#f3d9ee
